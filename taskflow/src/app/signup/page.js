        ...
        <div className="min-h-screen bg-[#F0FDFA] flex items-center justify-center p-6 selection:bg-teal-100">
            <div className="w-full max-w-md">
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 tracking-tight">Join the flow</h1>
                    <p className="text-slate-500 font-medium mt-2">Provision your workspace in seconds.</p>
                </div>
                ...
                <div className="bg-white/70 backdrop-blur-xl p-10 rounded-[2.5rem] border border-teal-600/10 shadow-2xl relative overflow-hidden">
                    {success ? (
                        <div className="py-12 text-center animate-in zoom-in-95 duration-500">
                            ...
                            <p className="text-slate-500 font-medium mb-8 text-sm">We've sent a verification link to <br /><b className="text-[#0D9488]">{email}</b></p>
                            <Link href="/login" className="text-xs font-black uppercase tracking-widest text-[#0D9488] hover:bg-teal-50 px-6 py-3 rounded-xl transition-all border border-teal-100">Back to Login</Link>
                        </div>
                    ) : (
                        <form onSubmit={handleSignup} className="space-y-6">
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Create Access Key</label>
                                <input
                                    type="password"
                                    value={password}
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full py-5 rounded-2xl bg-[#F97316] text-white font-black text-lg shadow-xl shadow-orange-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
                            >
                ...
                <p className="mt-10 text-center text-slate-400 font-bold text-sm">
                    Already have an account? <Link href="/login" className="text-[#0D9488] hover:underline">Log in</Link>
                </p>
            </div>
        </div>
    );
